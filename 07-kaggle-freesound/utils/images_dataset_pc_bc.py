...
class ImagesDataset:
    def __init__(
        self, audio_dir, image_dir, labels_csv, image_size=(200, 200), batch_size=32
    ):
    ...
    def mixup_audio_and_labels(self, file1, file2, alpha=0.2):
        ...
        mixed_label = np.clip(labels1 + labels2, 0, 1)  # Asegura que las etiquetas son binarias
        mixed_filename = f"{Path(file1).stem}_{Path(file2).stem}_mixed.wav"
        return mixed_audio, mixed_label, mixed_filename, sr1

    def mix_dataset(self, audio_dir):
        filenames = os.listdir(audio_dir)
        np.random.shuffle(filenames)
        mixed_labels = []
        for i in range(0, len(filenames) - 1, 2):
            file1 = os.path.join(audio_dir, filenames[i])
            file2 = os.path.join(audio_dir, filenames[i + 1])
    ...
    def split_train_val(self, train_dir, val_dir, test_size=0.2, random_state=78557):
    ...
    def get_train_and_val_dataset_mixed_up(self):
        ...
        self.split_train_val("C:/Users/Valen/Desktop/Laboratorio-2-TAA/data/train_folder_resampled_mixed", "C:/Users/Valen/Desktop/Laboratorio-2-TAA/data/val_folder_resampled_mixed")

        mixed_labels = self.mix_dataset("C:/Users/Valen/Desktop/Laboratorio-2-TAA/data/train_folder_resampled_mixed")

        self.process_audios_logmel("C:/Users/Valen/Desktop/Laboratorio-2-TAA/data/train_folder_resampled_mixed", "C:/Users/Valen/Desktop/Laboratorio-2-TAA/data/train_preproc_logmel_resampled_mixed")
        self.process_audios_logmel("C:/Users/Valen/Desktop/Laboratorio-2-TAA/data/val_folder_resampled_mixed", "C:/Users/Valen/Desktop/Laboratorio-2-TAA/data/val_preproc_logmel_resampled_mixed")
        ...
        mixed_labels_df['fname'] = mixed_labels_df['fname'].apply(lambda x: x.replace('.wav', ''))

        mixed_labels_df.set_index('fname', inplace=True)

        labels = pd.concat([labels, mixed_labels_df])

        print("Mixed labels dataframe:", labels.tail(200))
